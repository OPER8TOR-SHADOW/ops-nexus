...
// src/index.js
var VERSION = "14A.1.0";
var OAUTH_TOKEN_URL = "https://api.ebay.com/identity/v1/oauth2/token";
var EXCHANGE_GRANT_TTL_SECONDS = 180;
var consumedExchangeGrants = /* @__PURE__ */ new Map();
var JSON_HEADERS = {
...
__name(validateRequiredSecrets, "validateRequiredSecrets");
async function handleOAuthCallback(request, env) {
  ...
  const successDetail = handoffResult.mode === "callback" ? "OAuth token exchange succeeded and tokens were securely delivered to OPS Nexus Desktop." : handoffResult.mode === "browser_callback" ? "OAuth token exchange succeeded. Finalizing secure transfer to OPS Nexus Desktop on this device..." : "OAuth token exchange succeeded. Use the temporary secure exchange link in OPS Nexus Desktop.";
...
async function handleOAuthExchange(request, env) {
  ...
  if (!decrypted?.exp || nowMs >= Number(decrypted.exp) * 1e3) {
    return json({ message: "Grant expired" }, 410);
  }
  consumedExchangeGrants.set(digest, nowMs + EXCHANGE_GRANT_TTL_SECONDS * 1e3);
  return json({
    provider: "ebay",
...
__name(handleOAuthExchange, "handleOAuthExchange");
...
__name(handleDeletionChallenge, "handleDeletionChallenge");
...
__name(handleDeletionNotification, "handleDeletionNotification");
...
__name(summarizeDeletionPayload, "summarizeDeletionPayload");
...
__name(readVerificationToken, "readVerificationToken");
...
__name(isTrustedEbayHost, "isTrustedEbayHost");
function isValidOAuthCode(code) {
  return /^[A-Za-z0-9._~\-]{8,2048}$/.test(code);
}
__name(isValidOAuthCode, "isValidOAuthCode");
...
__name(isValidDesktopCallbackUrl, "isValidDesktopCallbackUrl");
async function validateOAuthState(state, env) {
  ...
  console.log(
    stateSecretExists ? `[OAuth State Worker] OPS_NEXUS_STATE_SECRET preview: ${stateSecretHead}...${stateSecretTail}` : "[OAuth State Worker] OPS_NEXUS_STATE_SECRET preview: (missing)"
  );
  if (!stateSecret) {
...
async function exchangeAuthorizationCode(code, request, env) {
  ...
  console.log(`[Token Exchange] eBay token endpoint: ${OAUTH_TOKEN_URL}`);
...
async function deliverOAuthTokens(tokenPayload, stateCheck, request, env) {
  ...
  if (callbackUrl) {
    ...
    if (!callbackResponse.ok) {
      console.log(`[Desktop Handoff] Early return: desktop_callback_http_${callbackResponse.status}`);
      return {
        ok: false,
        mode: "callback",
  ...
  async function createExchangeGrantUrl(tokenPayload2, request2, env2) {
    console.log("[Desktop Handoff] Mode chosen: callback");
    return { ok: true, mode: "callback" };
  }
  __name(createExchangeGrantUrl, "createExchangeGrantUrl");
...
__name(deliverOAuthTokens, "deliverOAuthTokens");
...
__name(isLocalhostCallbackUrl, "isLocalhostCallbackUrl");
...
__name(pruneConsumedExchangeGrants, "pruneConsumedExchangeGrants");
...
__name(encryptExchangeGrant, "encryptExchangeGrant");
...
__name(decryptExchangeGrant, "decryptExchangeGrant");
...
__name(isValidChallengeCode, "isValidChallengeCode");
...
__name(hashIpForLog, "hashIpForLog");
...
function redactSensitiveOAuthResponseText(value) {
  ...
  return raw.replace(/("access_token"\s*:\s*")[^"]*(")/gi, "$1[redacted]$2").replace(/("refresh_token"\s*:\s*")[^"]*(")/gi, "$1[redacted]$2");
}
__name(redactSensitiveOAuthResponseText, "redactSensitiveOAuthResponseText");
...
function renderOAuthResultPage(ok, details) {
  ...
  const stateLine = details.statePreview ? `<p><strong>State:</strong> ${escapeHtml(details.statePreview)}</p>` : "";
  const actionLine = details.action ? `<p><strong>Action:</strong> ${escapeHtml(details.action)}</p>` : "";
  const exchangeLine = details.exchangeUrl ? `<p><strong>Exchange URL:</strong> <a href="${escapeHtml(details.exchangeUrl)}">Open secure token exchange</a></p>` : "";
  const bridgeStatusLine = details.bridgeConfig ? '<p id="bridge-status"><strong>Desktop transfer:</strong> Waiting...</p>' : "";
  const bridgeScript = details.bridgeConfig ? `<script>
      (async () => {
...
__name(renderOAuthResultPage, "renderOAuthResultPage");
